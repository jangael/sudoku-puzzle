...
#James: resolove different font size between Mac and Winodws
#       Mac is based 72 dpi whereas Windows 96 dpi
#       So, calc scale factor to use it.
...
DEFAULT_CONFIG = {"fill":"",
      ...
      "text":"",
      "justify":"center",
                  "font": ("helvetica", round(12*fontDPI), "normal")}
...
class Text(GraphicsObject):
    ...
    def setSize(self, size):
        if 5 <= size <= 36:
            f,s,b = self.config['font']
            self._reconfig("font", (f,round(size*fontDPI),b))
        else:
            raise GraphicsError(BAD_OPTION)
...
class Entry(GraphicsObject):
    ...
    def setSize(self, size):
        if 5 <= size <= 36:
            self._setFontComponent(1,round(size*fontDPI))
        else:
            raise GraphicsError(BAD_OPTION)
